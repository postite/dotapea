	...
	,getFiles: function() {
		...
		return tink_core_Future.flatMap(tink_core_Promise.next(tink_core_Promise.next(tink_core_Promise.next(tink_core_Promise.next(tink_core_Promise.next(tink_core_Promise.next(asys_FileSystem.readDirectory(Parser.dotapeaPath),function(n) {
			...
			while(_g1 < n.length) {
				var v = n[_g1];
				++_g1;
				if(!sys_FileSystem.isDirectory(Parser.dotapeaPath + v)) {
					_g.push(v);
				}
			}
			return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Success(_g)));
			...
			if(_gthis.cleanIndexPages) {
				...
				while(_g1 < n.length) {
					var v = n[_g1];
					++_g1;
					if(_gthis.pages_lettres.indexOf(v) != -1) {
						_g.push(v);
					}
				}
				return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Success(_g)));
	...
	,exportToMd: function(path) {
		...
		return tink_core_Future.flatMap(tink_core_Promise.next(tink_core_Promise.next(tink_core_Promise.next(tink_core_Future.flatMap(asys_io_File.getContent(path),function(o) {
			...
			case 0:
				...
				if(_gthis.cleanIndexPages) {
					content = DotapeaTools.remImages(content);
					content = DotapeaTools.remIndex(content);
					content = DotapeaTools.remDoubleBreaks(content);
					content = DotapeaTools.remSpaceandBreaks(content);
				}
				...
				return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Success(haxe_ds_Option.Some(content))));
			case 1:
...
DotapeaTools.remImages = function(big) {
	var reg_r = new RegExp("^\\[!(.+)\\]\\((.+)?\\)","gm".split("u").join(""));
	return big.replace(reg_r,"");
};
DotapeaTools.remIndex = function(big) {
	var reg_r = new RegExp("\\*\\*_(.*)_\\*\\*","gm".split("u").join(""));
	return big.replace(reg_r,"");
};
DotapeaTools.remDoubleBreaks = function(big) {
	var reg_r = new RegExp("^\n{2,}","gm".split("u").join(""));
	return big.replace(reg_r,"");
};
